...
function removeFirstOccurrences(str, value) {
  const substringPosition = str.indexOf(value);

  if (substringPosition === -1) return str;

  const firstPart = str.slice(0, substringPosition);
  const secondPart = str.slice(substringPosition + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  if (valuePosition === -1 || value.length === 0) return str;

  let firstPart = str.slice(0, str.length - value.length - 1);
  const secondPart = str.slice(valuePosition + value.length);

  if (str[valuePosition - 1] === ' ') {
    firstPart += ' ';
  }
...
function countVowels(str) {
  ...
  return str.split('').reduce((acc, el) => {
    if (!vowels.includes(el)) return acc;
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(spaceIndex + 1, value.length - 1);
}
